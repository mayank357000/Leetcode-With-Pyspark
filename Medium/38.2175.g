...
team_id is the primary key for this table.
Each row of this table contains the ID of a national team 
and the change in its points in the global rankings.
...
WITH OriginalRanking AS (
    SELECT
        ...
        t.name,
        t.points,
        RANK() OVER (ORDER BY t.points DESC, t.name ASC) AS original_rank
    FROM TeamPoints t
),
UpdatedRanking AS (
    SELECT
        t.team_id,
        t.name,
        t.points + pc.points_change AS updated_points,
        RANK() OVER (ORDER BY t.points + pc.points_change DESC, t.name ASC) AS updated_rank
    FROM TeamPoints t
    JOIN PointsChange pc ON t.team_id = pc.team_id
)
SELECT
    o.team_id,
    o.name,
    o.original_rank - u.updated_rank AS rank_diff
FROM OriginalRanking o
JOIN UpdatedRanking u ON o.team_id = u.team_id;
